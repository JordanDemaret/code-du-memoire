...
def play_game(n_sessions, actions,state_next,states,prob, total_score):
	for i in range(n_sessions):
		indices = np.arange(DECISIONS)
		selected_indices = np.random.choice(indices, size=M, replace=False, p=prob[i]/np.sum(prob[i])) #Choose M indices with probabilities given by the neural network
		step=1
	...
	return actions, state_next,states, total_score, True	
...
def generate_session(agent, n_sessions, verbose = 1):	
	...
	prob = np.zeros([n_sessions, DECISIONS])	
	step = 0
	...
	while (True):
		...
		tic = time.time()
		actions, state_next,states, total_score, terminal = play_game(n_sessions, actions,state_next,states,prob, total_score)
...
if __name__ == "__main__":
	for i in range(100000): #1000000 generations should be plentyff
