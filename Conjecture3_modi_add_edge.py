...
if __name__ == "__main__":   
    ...
    if os.path.exists("3_2_M_value.txt") and os.path.exists("3_2_model.weights.h5"):
        model.build(input_shape=(None, observation_space)) 
        model.load_weights("3_2_model.weights.h5")  
        with open("3_2_M_value.txt", "r") as f:
            lines = f.readlines()
            for line in lines:
                if line.startswith("M ="):
                   if M == int(line.split("=")[1].strip()) :
                        print("Le nombre d'arêtes M ne correspond pas à celui du graphe de référence.")
                        print("Veuillez supprimer le fichier 3_2_M_value.txt et 3_2_model.weights.h5 pour continuer l'algorithme.")
                        exit( )
    ...
    for i in range(debut,100000): 
        ...
        model.save_weights(f"3_2_model.weights.h5")
        print(f"Les poids du modèle ont été sauvegardés dans 3_2_model.weights.h5")
        # Sauvegarder la valeur de M dans un fichier texte
        with open(f"3_2_M_value.txt", "w") as f:
            f.write(f"M = {M}\n")
            f.write(f"i = {i}\n")
            f.write(f"myRand = {myRand}\n")
        print(f"La valeur de M a été sauvegardée dans 3_2_M_value.txt")
