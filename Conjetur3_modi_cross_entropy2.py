...
N = 11 #number of vertices in the graph. Only used in the reward function, not directly relevant to the algorithm 
...
n_actions = M #The size of the alphabet. In this file we will assume this is 2. There are a few things we need to change when the alphabet size is larger,
			  #such as one-hot encoding the input, and using categorical_crossentropy as a loss function.
			  
observation_space = DECISIONS #Leave this at 2*DECISIONS. The input vector will have size 2*DECISIONS, where the first DECISIONS letters encode our partial word , and the next DECISIONS bits one-hot encode which letter we are considering now.
...
if __name__ == "__main__":   
    ...
    for i in range(debut,100000): #1000000 generations should be plentyff
        ...
        if (i%200==2): # To create a timeline, like in Figure 3
            ...
            for i, G in enumerate(listGraph):
                ...
                # --- Affichage et sauvegarde du complémentaire ---
                G_complement = nx.complement(G)
                plt.figure(figsize=(4, 4))
                nx.draw(G_complement, pos, with_labels=True, node_color='lightcoral', edge_color='gray')
                plt.title(f"Complémentaire itération n° {inter[i]}")
            plt.savefig(f"{myRand}/graphe_complement_{myRand}_iter_{inter[i]}.png")
            plt.close()
        ...
        with open(f"M_value.txt", "w") as f:
            f.write(f"M = {M}\n")
            f.write(f"i = {i}\n")
            f.write(f"myRand = {myRand}\n")
        print(f"La valeur de M a été sauvegardée dans {myRand}/M_value_{myRand}.txt")
